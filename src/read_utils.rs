...
pub struct ZipHandler<R: std::io::Read + std::io::Seek> {
...
mod tests {
    ...
    #[test]
    fn zip_file_handler() {
        let reader = File::open("tests/fixtures/file-handler.zip").unwrap();
        let mut file_handler = ZipHandler::new(reader, "tests/fixtures/file-handler.zip").unwrap();
